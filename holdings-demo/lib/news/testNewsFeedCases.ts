...
async function main() {
  ...
  const feed4 = await getRelevantNews(openPositions4, underlyingExposure4)
  ...
  const feed1 = await getRelevantNews(openPositions1, underlyingExposure1)
  const nvdaNews1 = feed1.find((item) => item.tickers.includes("NVDA"))
  logResult(
    "Case 1 — direct holding + ETF overlap",
    !!nvdaNews1 &&
      nvdaNews1.relevance === 2 &&
      feed1[0].tickers.includes("NVDA"),
    !nvdaNews1
      ? "NVDA news missing"
      : nvdaNews1.relevance !== 2
        ? `relevance=${nvdaNews1.relevance}`
        : !feed1[0].tickers.includes("NVDA")
          ? "NVDA not ranked at top"
          : undefined
  )
  ...
  const feed2 = await getRelevantNews(openPositions2, underlyingExposure2)
  logResult(
    "Case 2 — no matches",
  ...
  const feed3 = await getRelevantNews(openPositions3, underlyingExposure3)
  ...
  logResult(
    ...
    !nvdaNews3
      ? "NVDA news missing"
      : nvdaNews3.relevance !== 1
        ? `relevance=${nvdaNews3.relevance}`
        : undefined
  )
}
